    ...
    if (req.session.logged) {
        ...
        for (let i = 0; i < rol.length; i++) {
            ...
            if (rol[i] == "profesor") {
                async function getSubjects() {
                    let query =
                        "SELECT * FROM materia INNER JOIN profesor ON materia.profesor_usuario = profesor.nombreUsuario AND ? = materia.profesor_usuario";
                    return new Promise((resolve, reject) => {
                        con.query(query, [username], (error, rows) => {
                            return resolve(rows);
                        });
...
app.get("/cargarVistaEstudiante", (req, res) => {
    if (req.session.logged) {
        let username = res.locals.username;
        let query =
            "SELECT * FROM estudiante INNER JOIN materia ON materia.curso_descripcion = estudiante.descripcion_curso AND materia.profesor_usuario = ?";
        let cursos = [];
    ...
    if (req.session.logged) {
        ...
        for (let i = 0; i < rol.length; i++) {
            ...
            if (rol[i] == "profesor") {
                async function infoProfile() {
                    let query =
                        "SELECT * FROM profesor WHERE nombreUsuario = ?";
                    return new Promise((resolve, reject) => {
                        con.query(query, [username], (error, rows) => {
                            return resolve(rows);
                        });
...
app.get("/obtenerAprendizajes", (req, res) => {
    if (req.session.logged) {
        let query =
            "SELECT * FROM estudianteaprendizaje WHERE estudianteaprendizaje.estudiante_dni = ?";
...
app.get("/guardarAprendizajes", (req, res) => {
    let query =
        "UPDATE estudianteaprendizaje SET estado = ? WHERE estudianteaprendizaje.descripcion = ? AND estudianteaprendizaje.estudiante_dni = ?";
    let dni = 0;
    for (let key in req.query.data) {
        if (key == "dni") {
            ...
            for (let i = 0; i < req.query.data[key].length; i++) {
                con.query(
                    query,
                    [key, req.query.data[key][i].name, dni],
                    (error, rows, fields) => {
                        if (error) throw error;
                    }
                );
...
app.get("/agregarAprendizajes", (req, res) => {
    if (req.session.logged) {
        ...
        let query1 =
            "INSERT INTO aprendizajes (descripcion,id_materia,id_periodo) VALUES (?, ?, ?)";
        con.query(
            query1,
            [
                req.query.newLearningData[0],
                req.query.newLearningData[2],
                idPeriod,
            ],
            (error, rows, fields) => {
                if (error) throw error;
        ...
        con.query(
            query2,
            [req.query.newLearningData[4]],
            (error, rows, fields) => {
                if (error) throw error;
                // With a for I create each learning for each student
                for (let i = 0; i < rows.length; i++) {
                    let query3 =
                        "INSERT INTO estudianteaprendizaje (descripcion,estado,estudiante_dni,periodo_id,materia_id) VALUES (?,?,?,?,?)";
                    con.query(
                        query3,
                        [
                            req.query.newLearningData[0],
...
app.get("/generarReporte/:dni", (req, res) => {
    if (req.session.logged) {
        ...
        let query2 =
            "SELECT nombreMateria, nota1, nota2, nota3, nota4, nota5, nota6, nota7, nota8 FROM materia INNER JOIN nota ON materia.id = nota.id_materia WHERE nota.dni_alumno = ?";
        con.query(query1, [dni], (error, infoAlumno, fields) => {
            con.query(query2, [dni], (error, notasAlumno, fields) => {
                if (error) throw error;
                console.log(notasAlumno);
                ejs.renderFile(
                    "views/GenerateReport.ejs",
                    { datosAlumno: infoAlumno, dataNotas: notasAlumno },
                    (err, html) => {
                        if (err) throw err;
                        const options = {
                            format: "A4",
...
app.get("/obtenerEstadisiticasEstudiates", (req, res) => {
    ...
    let query =
        "SELECT * FROM estudiante INNER JOIN materia ON profesor_usuario = ? AND materia.curso_descripcion = estudiante.descripcion_curso INNER JOIN nota ON nota.dni_alumno = estudiante.dni AND nota.id_materia = materia.id";
...
app.post("/changeNumbersNotes", (req, res) => {
    ...
    for (let i = 0; i < infoToChange["data"].length; i++) {
        ...
        con.query(
            query,
            [
                infoToChange["data"][i]["value"],
                infoToChange["data"][i]["dni"],
                infoToChange["data"][i]["idSubject"],
            ],
            (error, rows, fields) => {
                if (error) throw error;
            }
...
app.post("/estudianteMateria", (req, res) => {
    ...
    let query =
        "SELECT * FROM estudiante INNER JOIN materia ON materia.nombreMateria = ? AND materia.curso_descripcion = estudiante.descripcion_curso INNER JOIN nota ON nota.dni_alumno = estudiante.dni AND nota.id_materia  = materia.id";
    ...
    uploads(req, res, function (err) {
        ...
        // SUCCESS
            ...
            sharp(req.file.path)
                .resize(width, heigth)
                .toFile(
                    "public/images/upload/avatars/avatar_" +
                        usuario +
                        "." +
                        extension,
                    (err) => {
                        if (!err) {
                            ...
                            let query =
                                "UPDATE usuario SET avatar = ? WHERE nombreUsuario = ?";
...
app.post("/crearMateria", (req, res) => {
    if (req.session.logged) {
        ...
        let query =
            "INSERT INTO materia (nombre,imagen,horasCatedra,profesor_usuario) VALUES (?,?,?,?);";
        con.query(
            query,
            [name, image, teachingHours, teacherUser],
            (error, rows, fields) => {
                if (error) throw error;
                res.redirect("/dashboard");
            }
...
app.post("/changePassword", (req, res) => {
    ...
    con.query(query, [res.locals.username], (error, rows, fields) => {
        ...
        bcrypt.compare(newPass, rows[0]["pass"], (err, row) => {
            if (!row) {
                let query =
                    "UPDATE usuario SET pass = ?, contraseña_cambiada = 'true' WHERE nombreUsuario = ?";
                bcrypt.hash(newPass, salt, (err, encrypted) => {
                    newPass = encrypted;
                    con.query(
                        query,
                        [newPass, res.locals.username],
                        (error, rows, fields) => {
                            if (error) throw error;
            ...
            bcrypt.compare(password, rows[0]["pass"], (err, row) => {
                if (row) {
                    ...
                    if (rows[0]["contraseña_cambiada"] === "true") {
                        ...
                        let query2 =
                            "SELECT nombre FROM rol INNER JOIN usuario ON ? = rol.nombreUsuario AND rol.nombreUsuario = usuario.nombreUsuario ";
                        con.query(query2, [username], (error, rows, fields) => {
                            ...
                            let query3 =
                                "SELECT frase,autor FROM frasesinspiradoras";
                            con.query(query3, (error, rows, fields) => {
                                i = Math.floor(Math.random() * rows.length);
                                aux = rows[i]["autor"].replace(/\b\w/g, (l) =>
                                    l.toUpperCase()
                                );
                                res.locals.inspirationalPhrase = [
                                    rows[i]["frase"],
                                    aux,
                                    rows[i]["frase"].length,
                                ];
                                req.session.inspirationalPhrase = [
                                    rows[i]["frase"],
                                    aux,
                                    rows[i]["frase"].length,
                                ];
